module variables_and_subs
	...
	double precision, parameter :: U_0 = 1
	...
	if (dir==1) then
		do col=2,N-1
			do row=2,N-1
				...
				d_u_dt(row,col) = -( u(row,col)*a + v(row,col)*b + ( 1/rho(row,col) )*c )
			enddo
		enddo
	! backward difference
	else if (dir == -1) then
	...
	if (dir==1) then
		do col=2,N-1
			do row=2,N-1
				...
				d_v_dt(row,col) = -( u(row,col)*a + v(row,col)*b + ( 1/rho(row,col) )*c )
			enddo
		enddo
	! backward difference
	else if (dir == -1) then
